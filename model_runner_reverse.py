...
if __name__ == '__main__':
    ...
    data["ensemble"]["runs"] = 6 #Change to 96
    data["ensemble"]["steps"] = 10000
    max_process_count = 96 #My computer

    iterations = 0
    count = int(max_process_count/ data["ensemble"]["runs"])
    out_model_base = f"scenarios/Verifier/Test_nokmob/final_output{space},{population},{R}"
    for iterations in range(count):
        out_model = out_model_base+f",iteration_{iterations}.csv"
        data["output"]["model_save_file"] = out_model
        if not (exists(out_model)):
            runModelScenario(data, 0, 0)

    diff_model = DiffEq(data)
    # Run constant diffmodel
    output_loc = f"scenarios/Verifier/Final_Const{space},{population},{R}.png"
    diff_model.beta = R
    diff_model.solve()
    diff_model.plot_constant(output_loc)

    result_loc = out_model

    #process the model data
    dataframes = []
    for i in range(count):

        out_model = out_model_base + f",iteration_{i}.csv"
        df = pd.read_csv(out_model)
        dataframes.append(df)

    df0 = pd.concat(dataframes)
